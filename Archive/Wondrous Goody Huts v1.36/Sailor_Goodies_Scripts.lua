...
-- // Teleport
			if switchTeleport == 1 then
				print("//// Wondrous Goody Type Activated: Teleport")
				local sTargetPlot = Sailor_Goody_Teleport(pUnit)
				if sTargetPlot ~= nil then
					...
					break
				else -- // Catch for nil: random unit spawner... Why would this ever happen? I dunno.
					print("Door's stuck! Spawning unit instead.")
					local sTargetUnit, iSpawnX, iSpawnY = Sailor_Goody_RandomUnit(pPlayer) -- // Call Random Unit Roller Function
					UnitManager.InitUnit(pOwner, sTargetUnit, iSpawnX, iSpawnY)
					pUnitAbility:ChangeAbilityCount(abilityTeleport, -switchTeleport)
...
-- Unit Yeeter
-- ///////////////////////////////////////////////////////
function Sailor_Goody_Teleport(pUnit)
	...
	for i, k in ipairs(sContinentsInUse) do
		...
		for i, v in ipairs(sContinentPlots) do
			local sPlot = Map.GetPlotByIndex(v)
			if not sPlot:IsImpassable() and not sPlot:IsCity() and not sPlot:IsWater() then
