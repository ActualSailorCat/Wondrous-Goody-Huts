...
function Sailor_Goody_RandomUnit(pPlayer)
    ...
    -- // Unit collection...
    local tValidUnits = {}
    for i, tRow in ipairs(DB.Query("SELECT * FROM Units WHERE " .. sailorWaterSwitch .. " AND ReligiousStrength = 0 AND TraitType NOT NULL AND UnitType NOT LIKE 'UNIT_HERO%' AND UnitType NOT IN ('UNIT_ARCHAEOLOGIST', 'UNIT_SPY', 'UNIT_NATURALIST', 'UNIT_ROCK_BAND', 'UNIT_BARBARIAN_RAIDER')")) do
