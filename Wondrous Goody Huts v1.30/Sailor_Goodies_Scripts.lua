...
--[[ don't look at this
local tSailorWonderTable = {}
local sailorPlotCount = Map.GetPlotCount()
local sailorCount = 1
while sailorCount <= sailorPlotCount do
	local pPlot = Map.GetPlotByIndex(sailorCount)
	if pPlot:IsNaturalWonder() == true then
		local pPlotFeatureIndex = pPlot:GetFeatureType()
		local pPlotFeatureType = GameInfo.Features[pPlotFeatureIndex].FeatureType
		print(pPlotFeatureType)
		table.insert(tSailorWonderTable, pPlot)
	end
	...
	if eOwner == -1 then
		...
		if unitList ~= nil then
			for i, pUnit in ipairs(unitList) do
				...
				local switchFormation = pUnitAbility:GetAbilityCount("ABILITY_SAILOR_GOODY_FORMATION")
...
-- / Sight
				if switchSightBomb >= 1 then
					...
					unitWildernessStatus = Game:GetProperty(pOwner .. "Wilderness" .. unitID)
					if switchSightBomb >= 1 then
						if unitWildernessStatus ~= nil then
							print("Door's stuck! Spawning unit instead.")
							local sTargetUnit, iSpawnX, iSpawnY = Sailor_Goody_RandomUnit(pPlayer) -- / Call Random Unit Roller Function
							UnitManager.InitUnit(pOwner, sTargetUnit, iSpawnX, iSpawnY)
							if pPlayer:IsHuman() then
							...
							Game:SetProperty(pOwner .. "Wilderness" .. unitID, 1)
						end
						newunitWildernessStatus = Game:GetProperty(pOwner .. "Wilderness" .. unitID)
					end
				end
-- / Formation
				if switchFormation == 1 then
					local pUnitFormation = pUnit:GetMilitaryFormation()
					if pUnitFormation > 1 then
						print("Door's stuck! Spawning unit instead.")
						local sTargetUnit, iSpawnX, iSpawnY = Sailor_Goody_RandomUnit(pPlayer) -- / Call Random Unit Roller Function
						UnitManager.InitUnit(pOwner, sTargetUnit, iSpawnX, iSpawnY)
						if pPlayer:IsHuman() then
							Game.AddWorldViewText(playerID, Locale.Lookup("[COLOR_LIGHTBLUE]Unit has formation! Spawning unit instead.[ENDCOLOR]"), iX, iY, 0)
						end
						pUnitAbility:ChangeAbilityCount("ABILITY_SAILOR_GOODY_FORMATION", -switchFormation)
						break
		...
		for dx = (iCityRadius * -1), iCityRadius do
			for dy = (iCityRadius * -1), iCityRadius do
				...
				if pPlotNearCity and (pPlotNearCity:GetOwner() == iCityOwner) and (pCity == Cities.GetPlotPurchaseCity(pPlotNearCity:GetIndex())) then
					...
					--print("Feature: ", pPlotFeatureType)
					if resourceSwitch == 1 then
						if (not pPlotNearCity:IsMountain()) and (pPlotFeatureType ~= "FEATURE_GEOTHERMAL_FISSURE") and (pPlotNearCity:GetResourceType() == -1) and (not pPlotNearCity:IsNaturalWonder()) then
							--print("Adding plot, ", pPlotNearCity)
							--print("With feature: ", pPlotFeatureType)
                ...
                if iNumberofTiles > 0 then
                    ...
                    for i, pTile in ipairs(pCityPlots) do
                        if i == iRandom2 then
                            ...
                            for i, tRow in ipairs(DB.Query("SELECT ImprovementType FROM Improvements WHERE RemoveOnEntry = 0 AND ImprovementType NOT IN ('IMPROVEMENT_SAILOR_WATCHTOWER', 'IMPROVEMENT_CORPORATION', 'IMPROVEMENT_INDUSTRY') AND ((ImprovementType IN (SELECT ImprovementType FROM Improvements WHERE PrereqTech IS NULL AND PrereqCivic IS NULL)) OR ((ImprovementType IN (SELECT ImprovementType FROM Improvements WHERE PrereqTech IN (" .. tTechValid:sub(1,-3) .. "))) OR (ImprovementType IN (SELECT ImprovementType FROM Improvements WHERE PrereqCivic IN (" .. tCivicValid:sub(1,-3) .. ")))))")) do
